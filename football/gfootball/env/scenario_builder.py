...
class Scenario(object):
  ...
  def __init__(self, config, task: dict = None):
    ...
    if task is not None and "scenario" not in task:
      self._scenario_cfg.game_duration = task["game_duration"]
      self._scenario_cfg.left_team_difficulty = task["left_team_difficulty"]
      self._scenario_cfg.right_team_difficulty = task["right_team_difficulty"]
      self._scenario_cfg.deterministic = task["deterministic"]
      self._scenario_cfg.offsides = True
      self._scenario_cfg.end_episode_on_score = task["end_episode_on_score"]
      self._scenario_cfg.end_episode_on_out_of_play = task["end_episode_on_out_of_play"]
      self._scenario_cfg.end_episode_on_possession_change = task["end_episode_on_possession_change"]

      self.SetBallPosition(task["ball_pos"][0], task["ball_pos"][1])

      self.SetTeam(Team.e_Left)
      self.AddPlayer(-1.000000, 0.000000, e_PlayerRole_GK, controllable=False)
      for i in range(task["num_teammates"]):
        self.AddPlayer(task["teammate_pos"][i][0], task["teammate_pos"][i][1], e_PlayerRole_CM)

      self.SetTeam(Team.e_Right)
      self.AddPlayer(-1.000000, 0.000000, e_PlayerRole_GK, controllable=False)
      for i in range(task["num_opponents"]):
        self.AddPlayer(task["opponent_pos"][i][0], task["opponent_pos"][i][1], e_PlayerRole_CM)

    elif task is not None and "scenario" in task:
      try:
        scenario = importlib.import_module('gfootball.scenarios.{}'.format(task["scenario"]))
      except ImportError as e:
        logging.error('Loading scenario "%s" failed' % task["scenario"])
        logging.error(e)
        sys.exit(1)
