...
class FootballEnvCore(object):
  ...
  def _reset(self, animations, inc, task: dict = None):
    global _unused_engines
    ...
    self._steps_time = 0
    self._step = 0
    self._config.NewScenario(inc=inc, task=task)
  ...
  def reset(self, inc=1, task: dict = None):
    ...
    self._step_count = 0
    self._trace = trace
    self._reset(self._env.game_config.render, inc=inc, task=task)
