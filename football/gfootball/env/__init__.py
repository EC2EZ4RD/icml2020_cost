...
def create_environment(env_name='',
  ...
  """
  assert env_name
  task = other_config_options.get("task", None)
  scenario_config = config.Config(values={'level': env_name}, task=task).ScenarioConfig()
  ...
  c = config.Config(values=config_values, task=task)

  env = football_env.FootballEnv(c, task=task)
