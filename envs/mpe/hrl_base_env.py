...
class BaseHRLMPE(BaseMPE, MultiAgentEnv):
    def __init__(self, env_config):
        ...
        self.low_level_observation_space = gym.spaces.Box(
            low=-np.inf, high=np.inf, shape=(self.observation_space.shape[0] + self.context_size,))
        self.low_level_action_space = self.action_space

    def reset(self):
        self.env_obs = BaseMPE.reset(self, config=self.env_config)
    ...
    def _low_level_step(self, action_dict):
        self.env_obs, self.env_rewards, done, self.env_info = \
            BaseMPE.step(self, self.low_level_actions(action_dict))
