...
class MultiAgentEnv(gym.Env):
    ...
    def __init__(self, world, reset_callback=None, make_callback=None, reward_callback=None,
        ...
        for agent in self.agents:
            ...
            if self.discrete_action_space:
                ...
                u_action_space = spaces.Box(low=-agent.u_range, high=+agent.u_range, shape=(self.world.dim_p,),
    ...
    def reset(self, config):
        # make a new world
        self.world = self.make_callback(config)
        # reset world
        ...
        if config["scenario_name"] == "simple_spread":
            for i, agent in enumerate(self.agents):
                agent.state.p_pos = config["task"][i]
                agent.state.p_vel = np.zeros(self.world.dim_p)
                agent.state.c = np.zeros(self.world.dim_c)
            for i, landmark in enumerate(self.world.landmarks):
                landmark.state.p_pos = config["task"][i + config["num_agents"]]
                landmark.state.p_vel = np.zeros(self.world.dim_p)
            self._reset_render()
        if config["scenario_name"] == "push_ball":
            for i, agent in enumerate(self.world.agents):  # agent+box
                agent.state.p_pos = config["task"][i]
                agent.state.p_vel = np.zeros(self.world.dim_p)
                agent.state.c = np.zeros(self.world.dim_c)
            for i, landmark in enumerate(self.world.landmarks):
                landmark.state.p_pos = config["task"][i + len(self.world.agents)]
                landmark.state.p_vel = np.zeros(self.world.dim_p)
            self._reset_render()
    ...
    def _get_state(self):  # get state of all entities
