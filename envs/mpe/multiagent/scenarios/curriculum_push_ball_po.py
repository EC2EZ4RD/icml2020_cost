...
class Scenario(BaseScenario):
    def make_world(self, config):
        ...
        world.dim_c = 2
        self.num_people = config["num_agents"]
        self.num_boxes = config["num_agents"]
        num_landmarks = config["num_agents"]
        self.num_agents = self.num_boxes + self.num_people  # deactivate "good" agent
        self.num_observable_agents = config["num_observable_agents"]
        assert self.num_observable_agents < self.num_agents
        # add agents
        ...
        for i, agent in enumerate(world.agents):
            ...
            agent.collide = True
            agent.silent = True
            agent.adversary = True if i < self.num_people else False  # people.adversary = True     box.adversary = False
            ...
            agent.action_callback = None if i < self.num_people else self.box_policy  # box有action_callback 即不做动作
    ...
    def observation(self, agent, world):
        ...
        for entity in world.landmarks:  # world.entities:
            ...
            landmark_pos.append(tmp_pos)  # whether covered
        landmark_pos = heapq.nsmallest(self.num_observable_agents, landmark_pos, key=lambda s: np.sum(np.square(s)))
        ...
        adv_pos = heapq.nsmallest(self.num_observable_agents, adv_pos, key=lambda s: np.sum(np.square(s)))
        good_pos = heapq.nsmallest(self.num_observable_agents, good_pos, key=lambda s: np.sum(np.square(s)))
    ...
    def get_info(self, world):
        ...
        success = False
        for l in world.landmarks:
            dists = [np.sqrt(np.sum(np.square(a.state.p_pos - l.state.p_pos))) for a in world.agents if
                     a.adversary == False]
