...
from agents.att_com import PPOComCurriculumTorchPolicy
from envs.gfootball import FootballCurriculumHierarchicalComEnv, eval_fn
...
EXAMPLE_USAGE = """
python run_curriculum_hrl_att_com_gfootball.py
"""
...
def create_args():
    ...
    parser.add_argument(
        "-f",
        "--config-file",
        default="./configs/gfootball_att_com_curriculum_hrl.yaml",
        type=str,
    ...
    parser.add_argument("--teacher", type=str,
                        choices=["agent_num_bandit", "discrete_bandit", "simple_bandit", "task_wise_bandit",
                                 "eval_on_5v5_bandit", "contextual_bandit", "time_teacher"], default=None)
...
if __name__ == "__main__":
    ...
    teacher = TEACHER_LIST[args.teacher].options(
        name="teacher", num_cpus=2, num_gpus=0).remote(config=args.teacher_config, seed=args.seed)
    ...
    register_env(env_name, lambda cfg: FootballCurriculumHierarchicalComEnv(cfg))
    ...
    temp_env = FootballCurriculumHierarchicalComEnv(env_config)
        ...
        "multiagent": {
            "policies": {
                "high_level_policy": PolicySpec(
                    policy_class=PPOComCurriculumTorchPolicy,
